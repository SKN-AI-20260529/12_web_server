...
def compare_author_queries():
    #  기본 방식, 최적화 방식 비교
    #  - 두 측정은 별도의 QuerySet을 만들어서 서로 값을 공유하지 않는 상태를 만듦

    # QuerySet: select결과의 각 행을 저장한 Model 객체 집합
    """작성자 접근의 N+1 쿼리와 select_related 결과를 비교한다."""
    plain = _measure(

        # Post 테이블 모든 데이터 중 pk 오름차순으로 3행만 조회
        # - Post는 Author와 N:1 (FK) 관계를 맺고 있음
        # - Django ORM은 Model(Post) 조회 시 자동으로 관계를 맺은 테이블(Author)을 조회함
        #  -> 이때, 조회 방식은 Post의 각 행에 접근하며 Author의 pk 값을 얻어와 Author 테이블에서 select 1회 시도
        #  -> Post 1회, Post 조회 행 수만큼 Author 조회 (3회)
        #      totalL: 4회
...
def compare_comment_queries():
    ...
    optimized = _measure(
        ...
        #  - Post를 1회 조회하고, 관계를 맺은 Comment도 1회만 조회 후 두 주회 결과를 장고 서버 메모리에서 연결하는 작업을 수행
...
def show_comment_counts():
    ...
    with CaptureQueriesContext(connection) as captured:
        rows = [(post.title, post.comment_count) for post in (
            ...
            #  - 단, Comment조회 시 본문 전체가 아닌 행의 개수만 Count
